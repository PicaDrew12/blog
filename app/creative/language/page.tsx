...
export default async function LanguageListPage() {
  const allWorks = await getAllCreativeWorksByType('language')
  const collections = await getCreativeWorksByTypeAndCollection('language')
  ...
  if (allWorks.length === 0) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-16">
        ...
        <p className="mt-4 text-neutral-600">No languages yet in this category.</p>
      </main>
    )
  }
        ...
        <section key={collectionName} className="mt-12">
          ...
          <div className="space-y-6">
            {works.map((work) => (
              <Link
                key={work.slug}
                href={`/creative/language/${work.slug}`}
