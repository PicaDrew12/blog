...
export default async function PoemListPage() {
  const allWorks = await getAllCreativeWorksByType('poem')
  const collections = await getCreativeWorksByTypeAndCollection('poem')
  ...
  if (allWorks.length === 0) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-16">
        ...
        <h1 className="text-3xl font-bold tracking-tight">Poems</h1>
        <p className="mt-4 text-neutral-600">No poems yet in this category.</p>
      </main>
    )
  }
