...
export default async function ProseListPage() {
  const allWorks = await getAllCreativeWorksByType('prose')
  const collections = await getCreativeWorksByTypeAndCollection('prose')
  ...
  if (allWorks.length === 0) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-16">
        ...
        <h1 className="text-3xl font-bold tracking-tight">Prose</h1>
        <p className="mt-4 text-neutral-600">No prose yet in this category.</p>
      </main>
    )
  }
